...
export async function runMembershipBackfill({ client, apply = false, projectId } = {}) {
  ...
  const report = {
    ...
    migrated: 0,
    alreadyMigrated: 0,
    skippedMissingOrInvalidEmail: 0,
  ...
  for (const member of legacy) {
    ...
    if (!validEmail(email)) {
      report.skippedMissingOrInvalidEmail += 1;
      continue;
    }
    if (namesByEmail.get(email)?.size !== 1) {
      report.skippedAmbiguousIdentity += 1;
      continue;
    }
    if (!role) {
      report.skippedUnknownRole += 1;
      continue;
    }
    ...
    const existingMembership = existingUser
      ? await client.projectMembership.findUnique({
          where: { projectId_userId: { projectId: member.projectId, userId: existingUser.id } }
        })
      : null;
  ...
  for (const id of projectIds) {
    ...
    const canonicalOwners = await client.projectMembership.count({
      where: { projectId: id, role: 'OWNER', isActive: true }
    });
