...
export function createProjectAuthorizationMiddleware({ service = authorizationService } = {}) {
  return async function authorize(req, res, next) {
    try {
      if ((req.method === 'DELETE' && /^\/projects\/[^/]+$/.test(req.path)) || /^\/projects\/[^/]+\/github\/artifacts$|^\/projects\/[^/]+\/trace-links$|^\/requirements\/[^/]+\/traceability$|^\/tasks\/[^/]+\/traceability$|^\/github-artifacts\/[^/]+\/traceability$|^\/trace-links\/[^/]+$/.test(req.path)) return next();
