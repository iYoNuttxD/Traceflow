...
export const projectInvitationService = {
  async create(projectId, creatorId, { email, role }, requestId) {
    ...
    await auditService.recordOperational({ actorUserId: creatorId, projectId, requestId, action: 'PROJECT_INVITATION_CREATED', resourceType: 'ProjectInvitation', resourceId: invitation.id });
    return {
  ...
  async revoke(projectId, invitationId, actorUserId, requestId) {
    ...
    await auditService.recordOperational({ actorUserId, projectId, requestId, action: 'PROJECT_INVITATION_REVOKED', resourceType: 'ProjectInvitation', resourceId: invitationId });
  },
  async accept(token, user, requestId) {
    ...
    await auditService.recordOperational({ actorUserId: user.id, projectId: invitation.projectId, requestId, action: 'PROJECT_INVITATION_ACCEPTED', resourceType: 'ProjectInvitation', resourceId: invitation.id });
    return membership;
  }
};
