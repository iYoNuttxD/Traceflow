...
export const projectMembershipService = {
  ...
  async updateRole(projectId, membershipId, role, actorId, requestId) {
    const result = await projectMembershipRepository.updateRoleSafely(projectId, membershipId, role, buildAuditEvent({ actorUserId: actorId, projectId, requestId, action: 'PROJECT_MEMBER_ROLE_CHANGED', resourceType: 'ProjectMembership', resourceId: membershipId }));
  ...
  async deactivate(projectId, membershipId, actorId, requestId) {
    const result = await projectMembershipRepository.setActiveSafely(projectId, membershipId, false, buildAuditEvent({ actorUserId: actorId, projectId, requestId, action: 'PROJECT_MEMBER_DEACTIVATED', resourceType: 'ProjectMembership', resourceId: membershipId }));
    if (!result) throw notFound();
    if (result.lastOwner) throw lastOwner();
  ...
  async reactivate(projectId, membershipId, actorId, requestId) {
    const result = await projectMembershipRepository.setActiveSafely(projectId, membershipId, true, buildAuditEvent({ actorUserId: actorId, projectId, requestId, action: 'PROJECT_MEMBER_REACTIVATED', resourceType: 'ProjectMembership', resourceId: membershipId }));
    if (!result) throw notFound();
  ...
  async leave(projectId, userId, requestId) {
    ...
    const result = await projectMembershipRepository.setActiveSafely(projectId, membership.id, false, buildAuditEvent({ actorUserId: userId, projectId, requestId, action: 'PROJECT_MEMBER_LEFT', resourceType: 'ProjectMembership', resourceId: membership.id }));
    if (result?.lastOwner) throw lastOwner();
  ...
  async transferOwnership(projectId, targetMembershipId, actorId, requestId) {
    const target = await projectMembershipRepository.transferOwnership(projectId, actorId, targetMembershipId, buildAuditEvent({ actorUserId: actorId, projectId, requestId, action: 'PROJECT_OWNERSHIP_TRANSFERRED', resourceType: 'ProjectMembership', resourceId: targetMembershipId }));
