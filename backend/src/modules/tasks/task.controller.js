...
const context = (req) => ({ actor: req.auth.user, actorUserId: req.auth.user.id, requestId: req.requestId });
...
export const taskController = {
  ...
  unlinkCommit: asyncHandler(async (req, res) => {
    const commits = await taskService.unlinkCommit(req.params.id, req.params.commitId, context(req));
