...
export async function runPrivacyRetention({
  ...
  const filters = {
    ...
    privacyRequests: {
      ...
      updatedAt: { lt: daysAgo(configuration.privacyRequestRetentionDays, now) }
    },
  ...
  const counts = {
    ...
    personalDataExports: await client.personalDataExport.count({
      where: filters.personalDataExports
    })
  };
  if (apply) {
    await client.$transaction(async (tx) => {
      ...
      await tx.auditEvent.create({
        data: {
          ...
          result: 'SUCCESS',
          metadataJson: { count: Object.values(counts).reduce((sum, count) => sum + count, 0) },
