...
describe('RF41 — sugestões persistidas Commit → Task', () => {
  it('analisa histórico, aceita somente o padrão oficial e não reabre rejeições', async () => {
    ...
    await createCommit(prisma, project.id, {
      message: `[TASK-${task1.id}] [task-${task2.id}] [TASK-${task1.id}] [TASK-${foreignTask.id}]`
    });
    await createCommit(prisma, project.id, {
      message: `TASK-${task1.id} #${task1.id} [ISSUE-${task1.id}] [TASK-ABC]`
    });
    ...
    const rejectedCommit = await createCommit(prisma, project.id, {
      message: `[TASK-${task2.id}]`
    });
    await prisma.taskCommitSuggestion.create({
      data: {
        projectId: project.id,
        taskId: task2.id,
        commitId: rejectedCommit.id,
        status: 'REJECTED',
        reviewedAt: new Date(),
    ...
    const first = await owner
      .mutate('post', `/api/projects/${project.id}/traceability/commit-suggestions/scan`)
      .send({});
    expect(first).toMatchObject({
      status: 200,
      body: {
        scannedCommits: 4,
        detectedReferences: 5,
        createdSuggestions: 2,
        skippedSuggestions: 3
      }
    });
    ...
    expect(
      await prisma.taskCommitSuggestion.count({
        where: { commitId: rejectedCommit.id, status: 'PENDING' }
      })
    ).toBe(0);
    expect(
      await prisma.auditEvent.count({
        where: { projectId: project.id, action: 'TASK_COMMIT_SUGGESTIONS_SCANNED' }
      })
    ).toBe(2);
  });
  ...
  it('pagina DTO minimizado e aplica VIEWER+, MEMBER+ e isolamento', async () => {
    ...
    const list = await viewer.agent.get(
      `/api/projects/${project.id}/traceability/commit-suggestions?page=1&limit=1`
    );
    ...
    expect(
      (
        await viewer
          .mutate(
            'post',
            `/api/projects/${project.id}/traceability/commit-suggestions/${suggestion.id}/confirm`
          )
          .send({})
      ).status
    ).toBe(403);
    expect(
      (await outsider.agent.get(`/api/projects/${project.id}/traceability/commit-suggestions`))
        .status
    ).toBe(404);
  });
  ...
  it('filtra por taskId, preserva consulta geral e rejeita tarefa inválida ou de outro projeto', async () => {
    ...
    expect(
      (await viewer.agent.get(`/api/projects/${project.id}/traceability/commit-suggestions`)).body
        .pagination.total
    ).toBe(2);
    expect(
      (
        await viewer.agent.get(
          `/api/projects/${project.id}/traceability/commit-suggestions?taskId=${task1.id + 10000}`
        )
      ).status
    ).toBe(404);
    expect(
      (
        await viewer.agent.get(
          `/api/projects/${project.id}/traceability/commit-suggestions?taskId=${foreignTask.id}`
        )
      ).status
    ).toBe(404);
    expect(
      (
        await viewer.agent.get(
          `/api/projects/${project.id}/traceability/commit-suggestions?taskId=abc`
        )
      ).status
    ).toBe(400);
    ...
    expect(empty).toMatchObject({
      status: 200,
      body: { suggestions: [], pagination: { total: 0 } }
    });
  });
  ...
  it('confirma de forma idempotente, cria TaskCommit uma vez e audita', async () => {
    ...
    expect((await member.mutate('post', path).send({})).body).toMatchObject({
      changed: true,
    ...
    expect((await member.mutate('post', path.replace('/confirm', '/reject')).send({})).status).toBe(
      409
    );
    expect(await prisma.taskCommit.count({ where: { taskId: task.id, commitId: commit.id } })).toBe(
      1
    );
    expect(
      await prisma.auditEvent.count({
        where: { action: 'TASK_COMMIT_SUGGESTION_CONFIRMED', resourceId: String(suggestion.id) }
      })
    ).toBe(1);
  ...
  it('rejeita de forma idempotente sem criar TaskCommit e preserva a decisão', async () => {
    ...
    expect((await member.mutate('post', path.replace('/reject', '/confirm')).send({})).status).toBe(
      409
    );
    expect(await prisma.taskCommit.count({ where: { taskId: task.id, commitId: commit.id } })).toBe(
      0
    ...
    expect(
      await prisma.auditEvent.count({
        where: { action: 'TASK_COMMIT_SUGGESTION_REJECTED', resourceId: String(suggestion.id) }
      })
    ).toBe(1);
    expect(
      (
  ...
  it('bloqueia sugestão inconsistente entre projetos sem vínculo ou auditoria', async () => {
    ...
    expect(
      await prisma.auditEvent.count({
        where: { resourceId: String(suggestion.id), action: 'TASK_COMMIT_SUGGESTION_CONFIRMED' }
      })
    ).toBe(0);
    expect(
      (await prisma.taskCommitSuggestion.findUnique({ where: { id: suggestion.id } })).status
    ).toBe('PENDING');
  });
});
