...
describe('backfill E6', () => {
  it('trata base vazia, casos ambíguos, projeto sem owner e execução idempotente', async () => {
    ...
    const canonicalOwner = await prisma.user.create({ data: { name: 'Owner canônico', email: 'canonical@example.invalid', passwordHash: null } });
    await prisma.projectMembership.create({ data: { projectId: partial.id, userId: canonicalOwner.id, role: 'OWNER' } });
    await prisma.projectMember.createMany({ data: [
      { projectId: owned.id, name: 'Owner artificial', email: 'owner@example.invalid', role: 'DONO' },
      { projectId: owned.id, name: 'Sem e-mail', email: null, role: 'MEMBRO' },
      { projectId: manual.id, name: 'Member artificial', email: 'member@example.invalid', role: 'MEMBRO' },
      { projectId: manual.id, name: 'Identidade A', email: 'shared@example.invalid', role: 'MEMBRO' },
      { projectId: ambiguous.id, name: 'Identidade B', email: 'shared@example.invalid', role: 'MEMBRO' },
      { projectId: ambiguous.id, name: 'Papel inválido', email: 'role@example.invalid', role: 'UNKNOWN' },
      { projectId: partial.id, name: 'Membro parcial', email: 'partial@example.invalid', role: 'MEMRO' }
    ] });
    const dryRun = await runMembershipBackfill({ client: prisma });
    expect(dryRun).toMatchObject({ mode: 'dry-run', skippedMissingOrInvalidEmail: 1, skippedAmbiguousIdentity: 2, skippedUnknownRole: 2 });
    expect(dryRun.projectsWithoutEligibleOwner).toEqual(expect.arrayContaining([manual.id, ambiguous.id]));
    ...
    expect(await prisma.user.findUnique({ where: { email: 'owner@example.invalid' } })).toMatchObject({ passwordHash: null, mustSetPassword: true });
  });
});
