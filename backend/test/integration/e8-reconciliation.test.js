...
async function createLegacyTables() {
  await prisma.$executeRawUnsafe(
    'CREATE TABLE `TaskPullRequest` (`id` INTEGER NOT NULL AUTO_INCREMENT, `taskId` INTEGER NOT NULL, `pullRequestId` INTEGER NOT NULL, PRIMARY KEY (`id`))'
  );
  await prisma.$executeRawUnsafe(
    'CREATE TABLE `GithubArtifact` (`id` INTEGER NOT NULL AUTO_INCREMENT, `projectId` INTEGER NOT NULL, `type` VARCHAR(191) NOT NULL, `externalId` VARCHAR(191) NULL, `sha` VARCHAR(191) NULL, `title` VARCHAR(191) NULL, `description` TEXT NULL, `author` VARCHAR(191) NULL, `status` VARCHAR(191) NULL, `branch` VARCHAR(191) NULL, `url` VARCHAR(191) NULL, `createdAtGithub` DATETIME(3) NULL, `closedAtGithub` DATETIME(3) NULL, PRIMARY KEY (`id`))'
  );
  await prisma.$executeRawUnsafe(
    'CREATE TABLE `TraceLink` (`id` INTEGER NOT NULL AUTO_INCREMENT, `projectId` INTEGER NOT NULL, `sourceType` VARCHAR(191) NOT NULL, `sourceId` INTEGER NOT NULL, `targetType` VARCHAR(191) NOT NULL, `targetId` INTEGER NOT NULL, PRIMARY KEY (`id`))'
  );
}
...
describe('E8 reconciliação e contract definitivos', () => {
  ...
  it('reconcilia TaskPullRequest singular e permanece idempotente', async () => {
    ...
    await prisma.$executeRawUnsafe(
      'INSERT INTO `TaskPullRequest` (`taskId`, `pullRequestId`) VALUES (?, ?)',
      task.id,
      pullRequest.id
    );
    ...
    expect(
      (await runE8Reconciliation({ client: prisma, apply: true })).pending.taskPullRequests
    ).toBe(0);
  });
  ...
  it('detecta múltiplas PRs e bloqueia contract sem escolher vínculo', async () => {
    ...
    for (const number of [1, 2]) {
      const pullRequest = await prisma.pullRequest.create({
        data: { projectId: project.id, githubId: `pr-${number}`, number, title: `PR ${number}` }
      });
      await prisma.$executeRawUnsafe(
        'INSERT INTO `TaskPullRequest` (`taskId`, `pullRequestId`) VALUES (?, ?)',
        task.id,
    ...
    await expect(
      runE8Contract({ client: prisma, apply: true, consumers: noConsumers })
    ).rejects.toMatchObject({ code: 'E8_CONTRACT_BLOCKED' });
  });
  ...
  it('reconcilia GithubArtifact correspondente e convertível sem expor conteúdo', async () => {
    ...
    await prisma.$executeRawUnsafe(
      'INSERT INTO `GithubArtifact` (`projectId`, `type`, `sha`, `author`) VALUES (?, ?, ?, ?), (?, ?, ?, ?)',
      project.id,
      'COMMIT',
      'existente',
      'PII proibida',
    ...
    expect(first.before.artifacts).toMatchObject({
      matchedCommit: 1,
      convertibleCommit: 1,
      exclusiveRecords: 0
    });
    expect(
      await prisma.commit.findUnique({
        where: { projectId_hash: { projectId: project.id, hash: 'novo' } }
      })
    ).not.toBeNull();
  ...
  it('detecta GithubArtifact ambíguo e TraceLink desconhecido', async () => {
    ...
    await prisma.pullRequest.create({
      data: { projectId: project.id, githubId: 'outro', number: 1, title: 'PR B' }
    });
    await prisma.$executeRawUnsafe(
      'INSERT INTO `GithubArtifact` (`projectId`, `type`, `externalId`) VALUES (?, ?, ?)',
      project.id,
      'PR',
      '1'
    );
    await prisma.$executeRawUnsafe(
      'INSERT INTO `TraceLink` (`projectId`, `sourceType`, `sourceId`, `targetType`, `targetId`) VALUES (?, ?, ?, ?, ?)',
      project.id,
      'CUSTOM',
      1,
      'OTHER',
  ...
  it('materializa TraceLink reconhecido e libera contract idempotente', async () => {
    ...
    await runE8Reconciliation({ client: prisma, apply: true });
    expect(
      await prisma.taskCommit.findUnique({
        where: { taskId_commitId: { taskId: task.id, commitId: commit.id } }
      })
    ).not.toBeNull();
