...
describe('E8 contract canônico', () => {
  it('audita e reconcilia o vínculo singular Task–PullRequest', () => {
    ...
    expect(
      auditTaskPullRequests({ links, tasks: [task], pullRequests: [pullRequest] })
    ).toMatchObject({
      totalRecords: 1,
    ...
    expect(
      taskPullRequestReconciliationPlan({ links, tasks: [task], pullRequests: [pullRequest] })
    ).toEqual({
    ...
    const memberships = [
      {
        userId: 7,
        isActive: true,
        user: { name: 'Pessoa Artificial', email: 'pessoa@example.invalid' }
      }
    ];
    expect(resolveUniqueUserByName(' pessoa artificial ', memberships)).toEqual({
      status: 'MATCHED',
      userId: 7
    });
    expect(mapProjectMemberToMembership({ email: 'PESSOA@example.invalid' }, memberships)).toEqual({
      status: 'MATCHED',
      userId: 7
    });
  ...
  it('protege apply em desenvolvimento e produção, liberando banco de teste', () => {
    expect(
      assertMaintenanceDatabase({
        databaseUrl: 'mysql://u:p@localhost/traceflow_test',
        developmentDatabaseUrl: 'mysql://u:p@localhost/traceflow',
        apply: true
      })
    ).toContain('traceflow_test');
    expect(() =>
      assertMaintenanceDatabase({ databaseUrl: 'mysql://u:p@localhost/traceflow', apply: true })
    ).toThrow(/confirm-development/);
    expect(() =>
      assertMaintenanceDatabase({
        databaseUrl: 'mysql://u:p@localhost/traceflow_production',
        apply: true
      })
