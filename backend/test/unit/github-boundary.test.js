...
describe('fronteira GitHub da E9', () => {
  it('resolve a credencial apenas no backend e falha sem expor o token', () => {
    expect(
      createGithubCredentialProvider({
        environment: { githubToken: 'token-artificial' }
      }).getToken()
  ...
  it('normaliza Repository, Commit, PullRequest e Issue sem payload Octokit', () => {
    ...
    expect(mapGithubIssue({ id: 3, number: 3, title: 'Issue', labels: ['bug'] })).toMatchObject({
      githubId: '3',
      labels: ['bug']
    });
  ...
  it('pagina até o fim e propaga interrupção sem ocultar páginas anteriores', async () => {
    ...
    await expect(collectGithubPages(paginateGithub(fetchPage, { perPage: 2 }))).resolves.toEqual([
      1, 2, 3
    ]);
    expect(fetchPage).toHaveBeenNthCalledWith(2, { page: 2, perPage: 2 });

    const interrupted = paginateGithub(
      vi
        .fn()
        .mockResolvedValueOnce({ items: [1], hasNext: true })
        .mockRejectedValueOnce(new Error('interrompida')),
      { perPage: 1 }
    );
  ...
  it('factory fixa host/timeout e envia branch principal, state all e páginas', async () => {
    ...
    const endpoints = {
      users: {
        getAuthenticated: vi.fn(async () => ({ data: { login: 'owner', id: 1, type: 'User' } }))
      },
      repos: {
        get: vi.fn(async () => ({
          data: {
        ...
        listForAuthenticatedUser: vi.fn(async (params) => {
          calls.push(['repos', params]);
          return { data: [] };
        }),
        listCommits: vi.fn(async (params) => {
          calls.push(['commits', params]);
          return { data: [] };
        })
      },
      pulls: {
    ...
    await collectGithubPages(
      client.listCommitPages({ owner: 'owner', repo: 'repo', branch: 'trunk' })
    );
    await expect(
      collectGithubPages(
        client.listPullRequestPages({ owner: 'owner', repo: 'repo', branch: 'trunk' })
      )
    ).resolves.toEqual([expect.objectContaining({ githubId: '2', targetBranch: 'trunk' })]);
    await expect(
      collectGithubPages(client.listIssuePages({ owner: 'owner', repo: 'repo' }))
    ).resolves.toEqual([]);

    expect(OctokitClass).toHaveBeenCalledWith(
      expect.objectContaining({
        auth: 'token-artificial',
    ...
    expect(calls).toContainEqual([
      'pulls',
      expect.objectContaining({ state: 'all', base: 'trunk', page: 1 })
    ]);
