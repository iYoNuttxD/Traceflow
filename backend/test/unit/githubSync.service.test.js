...
const repository = {
  githubRepositoryId: '200', name: 'repositorio-artificial', owner: 'usuario-artificial',
  fullName: 'usuario-artificial/repositorio-artificial',
  url: 'https://github.com/usuario-artificial/repositorio-artificial',
  defaultBranch: 'main', private: false, description: null
};
...
describe('githubSyncService com client e persistência substituídos', () => {
  beforeEach(() => {
    ...
    mocks.projectRepository.markGithubSyncSucceeded.mockResolvedValue({ ...project, githubSyncStatus: 'SINCRONIZADO' });
  ...
  it('pagina as três coleções, usa a branch validada e preserva o summary', async () => {
    ...
    expect(github.getRepository).toHaveBeenCalledWith(project.githubOwner, project.githubRepositoryName);
    expect(github.listCommitPages).toHaveBeenCalledWith(expect.objectContaining({ branch: 'main' }));
  ...
  it('preserva lotes anteriores, mantém lastSyncAt e marca falha sanitizada em paginação interrompida', async () => {
    ...
    const github = buildGithubDouble();
    github.listCommitPages.mockReturnValue((async function* interrupted() {
    ...
    expect(mocks.projectRepository.markGithubSyncFailed).toHaveBeenCalledWith(
      project.id, expect.any(Date), 'Falha de conexão com o GitHub.'
    );
  });
  ...
  it('impede sincronizações concorrentes do mesmo projeto na mesma instância', async () => {
    let release;
    const github = buildGithubDouble();
    github.getRepository.mockImplementation(() => new Promise((resolve) => {
