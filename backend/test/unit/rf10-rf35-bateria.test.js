...
// auditoria da Fase 1 encontrou sem prova: o teto da serie do burndown (I35),
// a equivalencia de offset nos calculadores (I36) e a resistencia do logger a
// injecao de linha (ASVS 16.4.1). Modulos puros + logger com escrita capturada:
// nenhum banco, nenhum relogio.
...
describe('teto da serie do burndown (I35)', () => {
  // O contrato vigente e TRUNCAMENTO silencioso em 180 pontos — nao recusa,
  // nao sinaliza. O teste congela esse comportamento; se um dia o teto virar
  // recusa ou aviso, este caso deve mudar junto, de proposito.
  it('trunca em 180 dias uma janela de 200', () => {
    const resultado = buildSprintBurndown({
      sprint: {
        ...
        projectId: 1,
        status: 'EM_ANDAMENTO',
        // 200 dias: 2026-01-01 ate 2026-07-20 (fim exclusivo).
    ...
    // 180 dias a partir de 01/01 terminam em 29/06 — o resto da janela e
    // descartado sem sinalizacao no payload.
...
describe('equivalencia de offset (I36)', () => {
  // Mesmos INSTANTES, grafias diferentes: tudo em Z num retrato, tudo em
  // -03:00 no outro. Se o resultado divergir, alguma comparacao esta usando o
  // dia local em vez do instante — exatamente o defeito que o ADR-010 D05
  // corrigiu na entrada.
  const sprintZ = {
    id: 10,
    projectId: 1,
  ...
  it('a evolucao nao muda quando as datas trocam Z por -03:00', () => {
    ...
    // E o instante serializado e o UTC, nao o offset de entrada.
      ...
      participations: [
        {
          ...
          currentStatus: 'CONCLUIDO',
          // 23h de 02/08 em UTC — em Brasilia ja seria 03/08? Nao: -03:00 de
          // 20h. A grafia abaixo, no outro retrato, e o MESMO instante.
...
describe('logger resiste a injecao de linha (ASVS 16.4.1)', () => {
  // O logger emite UMA linha JSON por evento. Nome de sprint com CRLF, escape
  // ANSI ou NUL nao pode quebrar essa linha nem chegar cru ao terminal: e o
  // JSON.stringify que blinda — todo C0 vira sequencia escapada. Este teste
  // congela essa garantia estrutural; se o formato deixar de ser JSON-line, a
  // protecao cai junto e este caso avisa.
  it('nome com CRLF, ANSI e NUL sai numa unica linha JSON, com os bytes escapados', () => {
    ...
    // Nenhum byte de controle cru (C0) na linha emitida — CR, LF, ESC e NUL
    // so podem existir escapados dentro do JSON.
    // eslint-disable-next-line no-control-regex
    expect(linha).not.toMatch(/[\u0000-\u001f]/);
    // A linha continua sendo JSON valido e o conteudo hostil vira dado inerte.
...
describe('guardas normativas do dominio (sobreviventes M05/M10 da bateria)', () => {
  // As duas guardas abaixo sao INALCANCAVEIS no fluxo atual do service — a
  // propria sprint nunca esta EM_ANDAMENTO quando a unicidade roda, e o retrato
  // do marco nunca chega vazio ao fechamento. A bateria de mutacao provou que
  // remove-las nao derruba teste nenhum. Elas continuam sendo a codificacao do
  // ADR-011 D05/D06, e o service pode ser reordenado amanha: o contrato da
  // FUNCAO e testado aqui para a guarda nao evaporar em silencio.
  ...
  it('allMilestoneSprintsConcluded exige ao menos uma sprint nao cancelada', async () => {
    ...
    // Marco sem sprint nenhuma nao "concluiu": nao ha entrega para afirmar.
    expect(allMilestoneSprintsConcluded([])).toBe(false);
    // So canceladas tambem nao: o trabalho inteiro foi abandonado, nao entregue.
