...
| GET          | `/tasks/:id/comments`                                                    | `id`; `page?`, `limit?`                                                                                              | `200`, `{taskId,total,comments,permissions,pagination}`                         |
...
| DELETE       | `/tasks/:id/comments/:commentId`                                         | ambos positivos                                                                                                     | `200`, `{message}`                                                              |
...
`TaskComment` é o modelo canônico. Autor vem exclusivamente de `req.auth.user`; nunca do body. `content` é obrigatório, sem espaços nas pontas e limitado a 2000 caracteres; conteúdo vazio recebe `400`. Listagem pagina do comentário mais recente para o mais antigo, 5 por página por padrão; a resposta inclui `permissions: {canComment,canModerate}` e, por comentário, `canEdit`/`canDelete` já resolvidos pelo backend para a sessão atual.
...
Comentário excluído permanece na listagem como marcador, preservando a linha do tempo exigida pelo RF31: o DTO mantém `id`, `createdAt` e `author`, devolve `content: null` e `editedAt: null`, expõe `deletedAt` e `deletedByModeration`, e fixa `canEdit`/`canDelete` em `false`. O conteúdo excluído não retorna para nenhum papel, inclusive quem moderou; permanece apenas no banco para auditoria. `deletedByModeration` é `true` somente quando a exclusão foi feita por alguém diferente do autor; sem `deletedById` registrado, o campo é `false` e nenhuma moderação é afirmada. `total` e `pagination` contam os marcadores, mantendo a paginação estável.
