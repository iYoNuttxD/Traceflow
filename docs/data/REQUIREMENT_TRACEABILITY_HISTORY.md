...
Leitura de projeção usa snapshot RepeatableRead e grupos de consultas Prisma por domínio, com seleção SQL correlacionada/indexada da execução corrente. Não carrega histórico inteiro de execuções, uploads ou arquivos. Uma execução corrente por caso; reteste mais recente por Defect. A medição anterior da carga detalhada foi **11 consultas para 1 e para 20 Requirements** com qualidade. A listagem paginada da PR #20 foi medida separadamente: 15 consultas de dados para projetos com 1/20/100 requisitos, ou 17 para 201, sempre detalhando no máximo 20 requisitos nessas fixtures. Não há N+1 por Requirement/TestCase/Defect.

**Limite de escala explícito — atualização PR #20:** a listagem percorre requisitos por cursor de ID, em lotes de até 200 IDs/títulos e respectivos agregados escalares no repository, sob RepeatableRead. A mesma policy de situação/lifecycle classifica esses agregados e aplica os filtros; somente os IDs da página carregam a projeção detalhada. Summary global e filteredSummary abrangem os conjuntos completos, sem usar o State persistido como autoridade. O custo dos agregados ainda cresce com o projeto, com consultas por lote, e as relações de um requisito individual podem ser numerosas; não é benchmark de projetos ilimitados. O histórico mantém paginação no banco. O grafo legado, inclusive sua limitação de métricas sobre a página identificada na Etapa 1, permanece intacto; o novo current não usa esse recorte. Evidência: [correções do review](../deliveries/S1_09_COPILOT_REVIEW_TARGETED_CORRECTIONS_REPORT.md).
