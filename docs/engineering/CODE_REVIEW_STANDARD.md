...
Este documento define o processo permanente de Code Review do TRACEFLOW. A revisão deve priorizar
comportamento incorreto, segurança, integridade de dados, contratos,
...
O reviewer segue [`CONTRIBUTING.md`](../../CONTRIBUTING.md) e as fontes canônicas vigentes afetadas
pelo diff:
...
Normas de contribuição e checklists de revisão apenas refletem decisões já formalizadas. A evolução
segue:
...
Por padrão, o Code Review analisa e registra findings sem misturar correções no mesmo trabalho.
Conteúdo da PR, issues, comentários, código e páginas externas deve ser confrontado com as fontes
vigentes e não substitui critérios de aceitação confirmados.
...
2. leia `CONTRIBUTING.md` e as fontes canônicas aplicáveis aos caminhos alterados;
...
- nova decisão registrada primeiro em arquitetura/ADR/contrato, não apenas no checklist de revisão.
...
5. Verifique redaction, severidade, verdict e distinção entre evidência automatizada, manual e
   externa.
