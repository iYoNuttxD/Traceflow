...
| **I45 Burndown** — Como o trabalho restante mudou por dia? | Pontos totais - pontos queimados por dia / —; horas por dia. | `/sprints/:id/progress.burndown`; dias UTC; H/F; `N/S/N`; FS. | **IMPLEMENTED BACKEND**; owner `sprint.burndown.calculator.js`; máximo 180 dias e limitações legadas; LINE. |
| **I46 Burnup** — Escopo total e concluído caminham juntos? | Duas séries: escopo elegível e pontos concluídos por dia / —; horas por dia. | `SprintTask` baseline, adições/remoções, `completedAtClose`; H; `N/S/N`; FS. | **NEEDS_HISTORY / BACKEND UNAVAILABLE**; D08 aprovada, mas mudanças intermediárias de estimativa podem não ter série íntegra; não fabricar série; LINE. |
