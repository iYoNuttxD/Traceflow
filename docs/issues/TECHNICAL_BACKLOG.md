...
| S104-F09 | Sanear marcos que o backfill deixou fora da janela da própria sprint | o fallback da migration `20260816120000_s104_domain_fixes` vinculou à última sprint do projeto os marcos cuja `dueDate` não caía em janela nenhuma. O ADR-010 D18 decidiu **tolerá-los**, validando só quem estava dentro — do contrário a sprint ficaria intocável até para renomear. Consequência: esses marcos seguem indefinidamente ancorados num período que não os contém, e nada os força a se ajustar | MÉDIA | relatório de marcos com `dueDate` fora de `[sprint.startDate, sprint.endDate)` por projeto, e decisão de produto sobre o que fazer com cada um (ajustar data, mover de sprint, criar sprint que os cubra). Não automatizar a correção: escolher data por conta própria inventaria histórico |
