...
| instalação GitHub App | navegador ↔ GitHub → callback Express → MySQL | state bruto no redirect; user token só em memória; metadados de instalação e autorizações persistidos | titular/GitHub/backend | state hashado/sessão/TTL, permissão usuário OWNER/ADMIN e acesso técnico App ACTIVE |
