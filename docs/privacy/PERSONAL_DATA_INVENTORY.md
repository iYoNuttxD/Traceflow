...
Consolidação técnica da E15 em 26/07/2026. O inventário descreve o tratamento implementado, mas não determina base legal nem comprova conformidade integral com a LGPD; essas decisões pertencem ao controlador com apoio jurídico.

| Dado | Finalidade | Origem | Persistência | Acesso | Retenção | Exclusão | Anonimização | Risco | Controle existente |
|---|---|---|---|---|---|---|---|---|---|
| Nome e e-mail da conta | identificação, login e comunicação | cadastro e perfil | `User` | titular; OWNER recebe visão autorizada de membros; SMTP recebe destinatário | enquanto a conta estiver ativa, observadas obrigações aplicáveis | fluxo do titular remove ou neutraliza conforme vínculos | substituição por valores neutros/aleatórios | enumeração, exposição e envio indevido | autenticação, RBAC, minimização de DTO e respostas uniformes |
...
| Sessão, CSRF e reset de senha | prova temporária de sessão/ação | backend e navegador | `Session`, `PasswordResetToken` | titular e backend | TTL técnico e limpeza operacional | revogação/expiração e remoção | não aplicável | sequestro de sessão/conta | token opaco hashado, cookies por ambiente, CSRF e rate limiting |
| Convite de projeto | admissão controlada | OWNER e e-mail do convidado | `ProjectInvitation`; compatibilidade em `Project.accessCode/inviteLink` | OWNER e destinatário conforme fluxo | TTL para convite canônico; legado até contract futuro | revogação/aceite; legado pendente de migração segura | e-mail pode ser neutralizado no fluxo de titular quando aplicável | vazamento e brute force do contrato legado | token canônico hashado, limiter e resposta uniforme; legado depreciado |
| Membership, papel e participação | autorização por projeto | convite, criação e administração | `ProjectMembership`; legado em `ProjectMember` | membros conforme papel; e-mail completo somente onde autorizado | ciclo da conta/projeto e trilha necessária | desativação lógica e fluxos de titular | neutralização quando compatível com integridade | BOLA/IDOR e elevação de privilégio | RBAC, 404 seguro, proteção do último OWNER e auditoria |
