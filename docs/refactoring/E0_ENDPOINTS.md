...
| GET `/api/projects/:projectId/commits` | `listProjectCommits` → commit service → commit repo → Project, Commit | Route `projectId`; query opcional `search` | 200 `{commits[]}` | RF03/RF06; busca em TaskForm/Task Details | IMPLEMENTADO |
| GET `/api/projects/:projectId/pull-requests` | `listProjectPullRequests` → PR service → PR repo → Project, PullRequest | Route `projectId`; query `search` por título/autor/número | 200 `{pullRequests[]}` | RF04/RF06/RF09; TaskForm/Task Details | IMPLEMENTADO |
| GET `/api/projects/:projectId/issues` | `listProjectIssues` → issue service → issue repo → Project, Issue | Route `projectId`; query `search` por título/autor/número | 200 `{issues[]}` | RF05/RF06/RF12; TaskForm/Task Details | IMPLEMENTADO |
...
| GET `/api/projects/:projectId/traceability/pull-request-coverage` | `getPullRequestCoverage` → homônimo → task repo → Project, Task | Route | 200 `{projectId,totalTasks,linkedTasks,coveragePercentage}` | RF09; API de cobertura, sem card agregado na Tasks Page | IMPLEMENTADO |
| GET `/api/projects/:projectId/traceability/commit-coverage` | `getCommitCoverage` → homônimo → task repo → Project, TaskCommit | Route | Mesmo formato de cobertura | RF11; API de cobertura, sem card agregado na Tasks Page | IMPLEMENTADO |
| GET `/api/projects/:projectId/traceability/issue-coverage` | `getIssueCoverage` → homônimo → task repo → Project, TaskIssue | Route | Mesmo formato de cobertura | RF12; API de cobertura, sem card agregado na Tasks Page | IMPLEMENTADO |
...
| PATCH `/api/tasks/:id/pull-request` | `linkPullRequest` → homônimo → task repo → Task, PullRequest | Body `{pullRequestId}`; `null`/vazio também desvincula | 200 `{message,task}` | RF09; TaskForm/Task Details | IMPLEMENTADO |
...
| POST `/api/tasks/:id/commits` | `linkCommit` → homônimo → task repo → Task, TaskCommit, Commit | Body `{commitId}`; valida projeto/duplicidade | 201 `{message,commits[]}` | RF11; TaskForm/Task Details | IMPLEMENTADO |
...
| POST `/api/tasks/:id/issues` | `linkIssue` → homônimo → task repo → Task, TaskIssue, Issue | Body `{issueId}`; valida projeto/duplicidade | 201 `{message,issues[]}` | RF12; TaskForm/Task Details | IMPLEMENTADO |
