...
- stream SSE autenticado `GET /api/projects/:projectId/events`;
...
| Information disclosure/BOLA      | Stream entrega evento de outro projeto ou preserva acesso revogado       | CRÍTICO | canal separado por `projectId`, membership na abertura, payload minimizado, encerramento em revogação/saída/logout e vida máxima de 15 min              | mudanças externas aos services dependem da expiração e reconexão         |
...
| Denial of service                | Conexões SSE ociosas, cliente lento ou buffering de proxy                | MÉDIO   | uma conexão por projeto/aba visível, close em hidden, heartbeat compartilhado, backpressure encerra stream e headers anti-buffering                     | limites/timeouts do reverse proxy precisam de validação operacional      |
...
- O publisher SSE em memória também é single-node; produção horizontal exige adapter de broker. O
  stream não transporta credenciais na URL, não aceita mutations e não mantém replay persistente.
