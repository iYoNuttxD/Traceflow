...
function TraceabilityItem({ children, href, removeLabel, removeTitle, onRemove }) {
  return (
    <div className="task-traceability-item">
      {href ? (
        <a className="task-pr-link" href={href} target="_blank" rel="noopener noreferrer">
          {children}
        </a>
      ) : (
        <span>{children}</span>
        ...
        <div className="task-traceability-group">
          ...
          {task.requirement ? (
            <TraceabilityItem
              ...
              removeTitle="Remover requisito"
              onRemove={() => onUnlinkRequirement(task.id)}
            >
        ...
        <div className="task-traceability-group">
          ...
          {task.pullRequest ? (
            <TraceabilityItem
              ...
              removeTitle="Remover pull request"
              onRemove={() => onUnlinkPullRequest(task.id)}
            >
            ...
            <div className="task-traceability-list">
              {task.commits.map((commit) => (
                <TraceabilityItem
                  ...
                  removeLabel="Remover commit vinculado"
                  removeTitle="Remover commit"
                  onRemove={() => onUnlinkCommit(task.id, commit.id)}
                >
                  {formatCommit(commit)}
                </TraceabilityItem>
              ))}
            ...
            <div className="task-traceability-list">
              {task.issues.map((issue) => (
                <TraceabilityItem
                  ...
                  removeLabel="Remover issue vinculada"
                  removeTitle="Remover issue"
                  onRemove={() => onUnlinkIssue(task.id, issue.id)}
                >
                  {formatIssue(issue)}
                </TraceabilityItem>
              ))}
