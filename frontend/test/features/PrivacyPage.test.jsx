...
vi.mock('../../src/features/auth/AuthContext.jsx', () => ({
  useAuth: () => ({
    user: { name: 'Pessoa artificial', email: 'person@example.invalid' },
    refresh: vi.fn()
  })
}));
...
function mockLoads() {
  apiMock.get.mockImplementation((path) => {
    if (path === '/account/sessions')
      return Promise.resolve({ data: { sessions: [{ id: 7, current: true }] } });
    if (path === '/account/audit-events')
      return Promise.resolve({
        data: {
          events: [{ id: 1, action: 'LOGIN_SUCCEEDED', occurredAt: '2026-01-01T00:00:00.000Z' }]
        }
      });
  ...
  it('edita o próprio perfil com confirmação de senha', async () => {
    ...
    await waitFor(() =>
      expect(apiMock.patch).toHaveBeenCalledWith(
        '/account/profile',
        expect.objectContaining({ name: 'Novo nome', currentPassword: 'SenhaSegura123' })
      )
    );
  });
});
